...
namespace DAL
{
    public class DatosPROFESOR : ConexionSql
    {
        SqlConnection Cnx;
        SqlCommand Cmd;
        ...
        public bool Agregar(PERSONA p)
        {
            ...
            try
            {
                strSql = "INSERT INTO profesor(DNI,Apellido,Nombre,Telefono,EmailPersonal,EmailInstitucional) VALUES(" + p.DNI + ",'" + p.APELLIDO + "','" + p.NOMBRE + "','" + p.TELEFONO + "','" + p.EMAILPERSONAL + "','" + p.EMAILINSTITUCIONAL + "');";
        ...
        {
            string strSql = "";
            using (SqlCommand sqlCommand = new SqlCommand("SELECT COUNT(*) from profesor where DNI='" + p.DNI + "';", Cnx))
            {
                ...
                strSql = "DELETE FROM profesor WHERE DNI=" + p.DNI + ";";
                Cmd = new SqlCommand(strSql, Cnx);
                Cmd.ExecuteNonQuery();
            }
            return true;
        ...
        public bool Modificar(PERSONA p)
        {
            ...
            using (SqlCommand sqlCommand = new SqlCommand("SELECT COUNT(*) from profesor where DNI='" + p.DNI + "';", Cnx))
            {
                ...
                strSql = "UPDATE profesor SET Apellido='" + p.APELLIDO + "', Nombre='" + p.NOMBRE + "', Telefono='" + p.TELEFONO + "', EmailPersonal='" + p.EMAILPERSONAL + "', EmailInstitucional='" + p.EMAILINSTITUCIONAL + "' WHERE DNI='" + p.DNI + "';";
                Cmd = new SqlCommand(strSql, Cnx);
                Cmd.ExecuteNonQuery();
            }
            return true;
        ...
        public bool BuscarPersona(PERSONA p, int DNI)
        {
            ...
            Cmd = new SqlCommand("SELECT * FROM profesor WHERE DNI=" + DNI, Cnx);
        ...
        public void ImportarPlano(string nombrearchivo)
        {
            try
            {
                ...
                while (archivo.Peek() > 1)
                {
                    ...
                    Cmd = new SqlCommand("INSERT INTO profesor (DNI,Apellido,Nombre,Telefono,EmailPersonal,EmailInstitucional) VALUES(" + registro[0] + ", '" + registro[1] + "','" + registro[2] + "', '" + registro[3] + "','" + registro[4] + "','" + registro[5] + "')", Cnx);
                    Cmd.ExecuteNonQuery();
                }
            }
            catch (Exception)
